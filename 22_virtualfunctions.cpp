...
public:
    Professor() { cur_id = ++idCounter; } // Assign unique ID in constructor
...
// Initialize static counter for Professor
...
public:
    Student() { cur_id = ++idCounter; } // Assign unique ID in constructor
    ...
    void getdata() override
    {
        ...
        marks[0] = sum; // Store sum in marks[0] for output
    }

    void putdata() override
    {
        cout << name << " " << age << " " << marks[0] << " " << cur_id << endl;
    }
};

// Initialize static counter for Student
...
and cur_id. It has two member functions: getdata and putdata. The function getdata should get the input from the user: the name, age, and the marks of the student in
...
The first line of input contains the number of objects that are being created. If the first line of input for each object is
...
, it means that the object is of the Student class, you will have to input the name, age and the marks of the student in
...
If the object is of the Student class, print the space separated name, age, the sum of the marks in

subjects and id on a new line.
